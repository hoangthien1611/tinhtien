...
import { checkValidActivityName, ValidActivityResult } from "./checkValidActivity"
...
class CreateActivityForm extends React.Component<CreateActivityFormProps, CreateActivityFormState> {
  ...
  private handleCreateActivity = () => {
    saveActivity(this.state.formValue.trim())
      .then((saveResponse) => {
  ...
  private routeToWelcome(url: String) {
    this.props.history.push("/welcome/" + url)
  }
      ...
      <div className="padding-top-10">
        <div className="center col5">
          <TextLineStateless
            ...
            autoFocus
          />
          <Button label="Create your activity" className="margin-top-5 center"
            disabled={this.state.validateResult != ""}
            onClick={() => this.handleCreateActivity()} />
        </div>
        <div className="margin-top-2 text-center warning">{this.state.validateResult}</div>
      </div>
    );
  }
}
